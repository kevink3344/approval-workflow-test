...
Since at least one organization is required for the system to function, a **Default Organization** is created automatically at seed time with slug `default`. All existing users without an organization are assigned to this default org.
...
Any existing admin user is automatically upgraded to `super_admin` during migration.

### Existing User Migration

All existing users (e.g., `alice@workflow.local`, `bob@workflow.local`, `charlie@workflow.local`) are assigned to the Default Organization. Existing workflows, approval groups, and approval requests are backfilled with the default organization's ID.
