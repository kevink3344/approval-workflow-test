...
### Phase 1.5 — Test Settings CRUD Endpoints

Before moving on to login endpoints, verify the settings storage layer works correctly using `curl`. This catches bugs early — missing tables, auth issues, or the env-var override logic.
...
**1. Read default login_mode (public — no auth required):**
```bash
curl -s http://localhost:3001/api/settings/login_mode | jq
```
Expect `{ key: "login_mode", value: "select" }` (or whatever default is configured).

**2. Read default maintenance_message (public):**
```bash
curl -s http://localhost:3001/api/settings/maintenance_message | jq
```
Expect `{ key: "maintenance_message", value: "" }` (or the default message).

**3. Update login mode as admin:**
```bash
curl -s -X PUT http://localhost:3001/api/settings/login_mode \
  -H "Authorization: Bearer $SUPER_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"value": "password"}' | jq
```
Expect `{ key: "login_mode", value: "password" }`.

**4. Verify the update persisted (public read):**
```bash
curl -s http://localhost:3001/api/settings/login_mode | jq
```
Expect `value: "password"`.

**5. Update maintenance message as admin:**
```bash
curl -s -X PUT http://localhost:3001/api/settings/maintenance_message \
  -H "Authorization: Bearer $SUPER_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"value": "Scheduled maintenance in progress. Please check back at 6 PM EST."}' | jq
```
Expect the message echoed back in `value`.

**6. Update login mode to maintenance:**
```bash
curl -s -X PUT http://localhost:3001/api/settings/login_mode \
  -H "Authorization: Bearer $SUPER_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"value": "maintenance"}' | jq
```
Expect `{ key: "login_mode", value: "maintenance" }`.

**7. Access control check — non-admin update should be rejected:**
```bash
curl -s -X PUT http://localhost:3001/api/settings/login_mode \
...
**8. Invalid value should be rejected:**
```bash
curl -s -X PUT http://localhost:3001/api/settings/login_mode \
  -H "Authorization: Bearer $SUPER_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
...
**9. Restore to default for clean state:**
```bash
curl -s -X PUT http://localhost:3001/api/settings/login_mode \
  -H "Authorization: Bearer $SUPER_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
...
**Checklist:**
- [ ] `GET /api/settings/login_mode` works without auth (public read)
- [ ] `GET /api/settings/maintenance_message` works without auth
- [ ] `PUT /api/settings/login_mode` (admin) updates the value
- [ ] `PUT /api/settings/maintenance_message` (admin) updates the message
- [ ] Public read confirms the update persisted
- [ ] Non-admin `PUT` gets 403
- [ ] Invalid value gets 400
